...
export const stepConfig = {
  ...
  triggers: [
    ...
    api('POST', '/tickets/triage', {
      ...
      responseSchema: {
        200: z.object({ ticketId: z.string(), assignee: z.string(), status: z.string() }),
...
export const { config, handler } = step(stepConfig, async (input, ctx) => {
  return ctx.match({
    ...
    cron: async () => {
      ...
      const allTickets = await ctx.state.list<{ id: string; status: string; priority: string; title: string }>('tickets')
      let swept = 0
